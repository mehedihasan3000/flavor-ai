...
- [x] Taste-profile recommendations (FR-TASTE-01..03)
...
- `[2026-09-03] [Post-MVP]` **AI Taste Matcher shipped (FR-TASTE-01..03).** Backend: additive `POST /ai/recipes/taste-match` (`types/index.ts` — `TASTE_PROFILE`/`TASTE_INTENSITY` enums, `TasteMatchInput`/`TasteMatchSuggestionSchema`; `aiService.ts` — `selectTasteCandidates` pre-filters published recipes into a bounded ~40-doc pool via the existing text index + `tags` field (no schema change), falling back to recently-published recipes when keyword matches are sparse, then `matchRecipesToTaste` sends a compact candidate summary to Groq to score/rank/explain matches, discarding any hallucinated `recipeId` outside the offered pool before hydrating full `Recipe` docs via `recipeController.toRecipeResponse` (now exported)). Mounted in `routes/ai.ts` behind `requireAuth`+`aiRateLimiter`, same failure-handling contract as the other AI endpoints. Documented as an "Additive (post-freeze)" entry in `docs/API_CONTRACT.md`. 18 new backend tests (`tests/tasteMatch.test.ts` — candidate selection/fallback/hallucination-filtering with a real `mongodb-memory-server` DB + mocked Groq fetch; `tests/aiController.test.ts` — route-level auth/validation/happy-path). Frontend: `lib/types.ts`/`lib/api.ts` additive `TasteProfile`/`TasteMatchInput`/`TasteMatchResult`/`matchRecipesToTaste()`; new `components/recipes/taste-match-panel.tsx` (taste chip multi-select, optional intensity + free-text notes, `AuthPrompt` gate for guests) surfaced as a "Match my taste" tab alongside the existing filters on `/recipes` (`?mode=taste`, sibling to the category/cuisine/diet browse mode — the AI-reranked result list is capped at `limit` rather than paginated, since it's a scored top-N, not a full corpus page); `RecipeCard` gained optional `matchScore`/`matchReason` props rendering a "NN% match" corner badge. 3 new frontend tests (`tests/tasteMatchPanel.test.tsx`). Backend: 225/225 tests ✓, lint ✓, build ✓. Frontend: 30/30 tests ✓, lint ✓, build ✓ (18 routes prerender).
