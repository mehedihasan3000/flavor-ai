...
- `[2026-09-21] [Lead]` **Photo Nutrition Vercel 413 root-cause fix (5–7 MB failed, 1–2 MB + >10 MB worked):**
  1. **Root cause:** Vercel Functions enforce a HARD 4.5 MB request/response body cap at the edge (`413 FUNCTION_PAYLOAD_TOO_LARGE`, no CORS headers — hence the CORS + `net::ERR_FAILED 413` + generic "Cannot reach" in `ERROR.md`). Base64 inflates ~33%: 5 MB → ~6.7 MB JSON, 7 MB → ~9.3 MB — rejected before Express/CORS run, while localhost (Express `15mb`) accepted them. 1–2 MB (~1.3–2.7 MB JSON) fit, so they worked; >10 MB took the canvas-compress path (landed ~1–3 MB JPEG) so they worked too. Only uncompressed 3–10 MB payloads fell in the gap. Not an image-size, timeout, memory, or model issue.
  2. **Fix (production-safe, quality-preserving):** `photo-dropzone.tsx` now compresses EVERYTHING over ~3 MB binary (new `needsCompressionForVercel` / `estimateJsonPayloadBytes` helpers) to a 2.8 MB budget (≈3.6 MB JSON, margin under 4.5 MB) — 1600px cap, JPEG ladder, white-fill for transparency, GIFs convert via first frame instead of being rejected. 1–2 MB untouched. Analyzer page adds a preflight JSON-size guard (clear local error instead of a bare 413) + explicit 413 handling; `lib/api.ts` maps 413 → `VALIDATION_ERROR` with a "photo too large" message. Backend unchanged behaviorally (`15mb` Express limit = non-Vercel runtimes only; added comment) + `vercel.json` `functions.api/index.ts.maxDuration: 60` so the 60s vision timeout can't be cut by the serverless default.
  3. **Size matrix (binary → sent JSON vs 4.5 MB cap):** 1 MB→1.33 ok as-is; 2 MB→2.67 ok as-is; 5/6/7/10/12/15 MB→3.56 compressed ok (raw would be 6.7/8.0/9.3/13.3/16/20 MB = 413). Backend 10 MB validation accepts all.
  4. **Verification:** frontend lint ✓, `photoAnalyzer` 15/15 ✓ (+6: copy + 7-size threshold + raw-vs-cap + untouched-small + compressed-budget), related frontend 25/25 ✓, build ✓ (18 routes); backend build ✓, lint 0 warnings ✓, photo/image suites 19/19 ✓.
  5. **Review follow-ups:** binary passthrough gate tightened 3 → 2.8 MB (kills the ~5 KB dead-end margin for large notes); status-0 + large-payload fallback added for edge-413-as-network-failure; stale 10 MB transport comments in `server.ts`/`imageService.ts` corrected to the Vercel-safe budget; GIF still-frame flattening disclosed in dropzone copy. `maxDuration: 60` within plan limits (Hobby max 300s). Re-verified: frontend lint/tests/build ✓, backend build/lint ✓.
- `[2026-09-21] [Lead]` **Photo Nutrition 502 root-cause fix (Groq 404 `qwen/qwen3.6-27b`, live-probed on the free-tier key):**
  1. **Evidence, not guesses:** `GET /openai/v1/models` shows 13 accessible models — `qwen3.8` present, `qwen3.6` absent (hence the 404). Probes proved: `3.8` serves vision plain (200 + real description) but 400s on `response_format: json_object` ("'messages' must contain the word 'json'"); `max_tokens: 2048` trips free-tier OTPM-1000 ("reduce max_tokens"); 900 truncates the nutrition JSON mid-object (parse fail); **1000 succeeds end-to-end** (URL + base64 paths, 5–6 foods).
  2. **Fix (`aiService.ts`):** removed dead `3.6` from default candidates (keys with access can append via new `GROQ_IMAGE_FALLBACK_MODELS` env); `qwen3.8` always called plain (no wasted 400); `max_tokens` 2048 → 1000; 401/403/404 break the variant loop instead of pointlessly retrying; 429 honored with one Retry-After backoff (≤10s, abort-aware); per-model `attempts[]` log replaces the misleading last-error-only message; 429-only exhaustion → 429 `RATE_LIMITED` ("busy, try again") instead of generic 502.
  3. **Config/docs:** `GROQ_MODEL_FOR_IMAGE` default + `.env.example` now `qwen3.8` (example was still `3.6`); `API_CONTRACT` failure line gains the 429 case; `AGENTS.md` vision-model rule corrected.
  4. **Verification:** backend build ✓, lint 0 warnings ✓, full suite 24 files / 221 tests ✓ (+4: plain-only call, 429-retry success, 429→RATE_LIMITED mapping, fallback walk + aggregated logging); live `analyzeFoodPhoto` via new code succeeds on URL and base64 inputs.
